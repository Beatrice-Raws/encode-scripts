...
def main_aa(clip: vs.VideoNode) -> vs.VideoNode:
    return clip_aa(clip=clip, descale_strength=0.8, kernel='bicubic', descale_height=900, bicubic_b=0, bicubic_c=0.5)
...
def main_dehalo(clip: vs.VideoNode) -> vs.VideoNode:
    return fd.fine_dehalo(clip=clip, darkstr=0, brightstr=0.8, showmask=0, rx=1.7, ry=1.7, thmi=65, thma=100, useMtEdge=True)

def denoise_deband(clip: vs.VideoNode, expr_multiplier: float = 4,
    thsad: int = 32, thsadc: int = 16, blksize: int = 8, overlap: int = 4,
    d_range: int = 8, y: int = 56, cb: int = 16, cr: int = 16, grainy: int = 64, grainc: int = 36) -> vs.VideoNode:
...
def credits_mask(clip: vs.VideoNode, b: float = 0, c: float = 0.5) -> vs.VideoNode:
    mask1 = core.std.Prewitt(clip.std.ShufflePlanes(0, vs.GRAY))
    mask2 = at.DiffRescaleMask(clip, kernel="bicubic", descale_h=900, b=b, c=c, mthr=25).ExprStr('0 if x < 10 else 255').std.Maximum().std.Maximum().std.Deflate()
...
def rescale_credit_mask(clip: vs.VideoNode) -> vs.VideoNode:
    return clip.at.DiffRescaleMask(descale_h=900, kernel='bicubic', b=0, c=0.5, mthr=30).std.Maximum().std.Inflate()
